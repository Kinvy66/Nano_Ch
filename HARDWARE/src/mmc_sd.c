...
{
	return SPIx_ReadWriteByte(SPI1,data);
}	  
//SD卡初始化的时候,需要低速
void SD_SPI_SpeedLow(void)
{
 	SPIx_SetSpeed(SPI1,SPI_BaudRatePrescaler_256);//设置到低速模式	
}
//SD卡正常工作的时候,可以高速了
void SD_SPI_SpeedHigh(void)
{
 	SPIx_SetSpeed(SPI1,SPI_BaudRatePrescaler_2);//设置到高速模式	
}
//SPI硬件层初始化
...
{			  	  
    ...
    while(len--)//开始接收数据
    {
        *buf=SPIx_ReadWriteByte(SPI1,0xFF);
        buf++;
    }
    //下面是2个伪CRC（dummy CRC）
...
{	
	...
	if(cmd!=0XFD)//不是结束指令
	{
		for(t=0;t<512;t++)SPIx_ReadWriteByte(SPI1,buf[t]);//提高速度,减少函数传参时间
