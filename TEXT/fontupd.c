...
//字库存放起始地址 
#define FONTINFOADDR 	1024*1024*4 					//WarShip STM32F103 V3是从12M地址以后开始存放字库
														//前面12M被fatfs占用了.
...
u8*const GBK2312_PATH="0:/FONT/GBK2312.FON";		//GBK2312的存放位置
u8*const ASCII_PATH="0:/FONT/ASCII.BIN";			//ASCII的存放位置
u8*const UNIGBK_PATH="0:/FONT/UNIGBK.BIN";		//UNIGBK.BIN的存放位置
 	...
 	if(rval==0)	 
	{
		switch(fx)
		{
			...
			case 1:
				...
				break;
			case 2:
				ftinfo.asc2addr=ftinfo.f16addr+ftinfo.gbk16size;	//GBK16之后，紧跟ASCII字库
				ftinfo.asc2size=fftemp->fsize;					//ASCII字库大小
				flashaddr=ftinfo.asc2addr;						//ASCII的起始地址
				break;
		} 
...
//	LCD_ShowString(x,y,240,320,size,"Updating GBK16.BIN  ");
	res=updata_fontx(ascii_path,2);			//更新ASCII
	if(res)return 3;
